    ...
    if ($autoLabelPoliciesFile) {
        ...
        if (Test-Path $importScript) {
            try {
                ...
                # Pass label GUID map from label import (generated by 06-Import-SensitivityLabels.ps1)
                $labelGuidMapFile = Join-Path $BackupPath "label-guid-map.json"
                if (Test-Path $labelGuidMapFile) {
                    $importParams['LabelGuidMapFile'] = $labelGuidMapFile
                    Write-Host "   📄 Using label GUID map: $labelGuidMapFile" -ForegroundColor Gray
                }
                # Enable auto SIT remapping by name
                $importParams['AutoBuildSitMap'] = $true
